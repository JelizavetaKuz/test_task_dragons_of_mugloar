...
export default {
  ...
  methods:{
    buy_item: function() {
      axios.post("https://dragonsofmugloar.com/api/v2/" + this.get_game_session.gameId + "/shop/buy/" + this.item.id)
          .then(response => {
            this.purchase = response.data;
            const update = {
              "lives": this.purchase["lives"],
              "gold": this.purchase["gold"],
              "level": this.purchase["level"],
              "turn": this.purchase["turn"]
            };
            ...
            this.makeToast(this.purchase, this.makeMessage(this.purchase))
          });

    },
    makeToast(purchase, message ) {
      this.$bvToast.toast( message, {
        title: purchase["shoppingSuccess"]? "Success!":"Fail!",
        toaster:"b-toaster-bottom-right",
        variant: purchase["shoppingSuccess"]?"success":"danger",
        solid: true,
        autoHideDelay: 5000,
        appendToast:true
      })
    ...
    makeMessage(pur){
      ...
      const vNodesMsgFail = "Failed to buy"+this.item["name"]
      return pur["shoppingSuccess"]? vNodesMsgSuccess: vNodesMsgFail;
    }
  },
  computed:{
    get_game_session(){
